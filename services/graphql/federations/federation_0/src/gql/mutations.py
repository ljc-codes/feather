...
class CreateMote(graphene.Mutation):
    '''
    ...
    ok = graphene.Boolean()
    mote = graphene.Field(lambda: MoteObject)
    ...
    def mutate(root, info,title,username,summary,content):
        ...
        author.motes.append(mote); author.save()
        # Returns ok status.
        ok = True
        return CreateMote(mote=mote,ok=ok)
