...
deploy_microservices="$(config_get deploy_services)"
...
if [ "$repo_owner" = '' ]; then
    ...
    echo Granting "$(config_get cloudbuild_serviceaccount_role)" access to the Cloud Build Service Account
    gcloud projects add-iam-policy-binding ${project_id} --member "serviceAccount:${project_number}@cloudbuild.gserviceaccount.com" --role "$(config_get cloudbuild_serviceaccount_role)"
    ...
    if [ "$deploy_microservices" = true ]; then
        ...
        gcloud builds submit ../services/client --config="../services/client/cloudbuild.yaml" & \
        gcloud builds submit ../services/graphql/gateway --config="../services/graphql/gateway/cloudbuild.yaml"  & \
        gcloud builds submit ../services/graphql/federations/federation_0 --config="../services/graphql/federations/federation_0/cloudbuild.yaml"  & \
        gcloud builds submit ../services/graphql/federations/federation_1 --config="../services/graphql/federations/federation_1/cloudbuild.yaml"  & \
        gcloud builds submit ../services/graphql/federations/federation_2 --config="../services/graphql/federations/federation_2/cloudbuild.yaml"  & \
